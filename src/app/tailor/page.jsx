...
export default function TailorPage() {
  ...
  const analyzeJobDescription = async () => {
    ...
    try {
      ...
      const { computeMatchScore } = await import("../../utils/ai");
  ...
  const generateAiSuggestions = (allKeywords, scoreDetails) => {
    ...
    if (scoreDetails.details.keywordMatch.missing.length > 0) {
      suggestions.push(
        `Adaugă aceste cuvinte cheie importante: ${scoreDetails.details.keywordMatch.missing
          .slice(0, 5)
          .join(", ")}`
      );
    }
    ...
    if (scoreDetails.details.completeness.missingFields.length > 0) {
      suggestions.push(
        `Completează câmpurile lipsă: ${scoreDetails.details.completeness.missingFields.join(
          ", "
        )}`
      );
    }
    ...
    if (scoreDetails.details.workExperience.score < 70) {
      suggestions.push(
        "Îmbunătățește descrierile experienței de muncă cu mai multe detalii și realizări cuantificabile"
      );
    }
  ...
  const optimizeResume = (keywords) => {
    ...
    if (optimized.professionalSummary) {
      optimized.professionalSummary = `Profesionist cu experiență în ${keywords
        .slice(0, 3)
        .join(", ")}. ${optimized.professionalSummary}`;
    }
    ...
    <div className="min-h-[calc(100vh-4rem)] bg-gradient-to-br from-slate-50 to-indigo-50 py-8 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <p className="text-center text-gray-600 mb-10 max-w-2xl mx-auto">
          Sistemele ATS (Applicant Tracking System) folosite de recrutori
          filtrează CV-urile înainte ca acestea să ajungă la ochii umani.
          Optimizează-ți CV-ul pentru a trece de aceste sisteme și a obține un
          interviu.
        </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white p-6 rounded-xl shadow-lg">
              ...
              <div className="mb-6">
                ...
                <div className="mb-4">
                  ...
                  <p className="text-sm mt-2">
                    {matchScore.total > 70
                      ? "Excelent! CV-ul tău are șanse mari să treacă de sistemele ATS."
                      : matchScore.total > 40
                      ? "Potrivire medie. Urmează sugestiile pentru a îmbunătăți scorul."
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-4">
                  ...
                  <div className="bg-gray-50 p-3 rounded-lg">
                    <div className="flex justify-between mb-1">
                      ...
                      <span className="text-sm font-bold">
                        {matchScore.details.keywordMatch.score}%
                      </span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div
                        className={`h-2 rounded-full ${
                          matchScore.details.keywordMatch.score > 70
                            ? "bg-green-500"
                            : matchScore.details.keywordMatch.score > 40
                            ? "bg-yellow-500"
                            : "bg-red-500"
                        }`}
                        style={{
                          width: `${matchScore.details.keywordMatch.score}%`,
                        }}
                      ></div>
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-50 p-3 rounded-lg">
                    <div className="flex justify-between mb-1">
                      ...
                      <span className="text-sm font-bold">
                        {matchScore.details.completeness.score}%
                      </span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div
                        className={`h-2 rounded-full ${
                          matchScore.details.completeness.score > 70
                            ? "bg-green-500"
                            : matchScore.details.completeness.score > 40
                            ? "bg-yellow-500"
                            : "bg-red-500"
                        }`}
                        style={{
                          width: `${matchScore.details.completeness.score}%`,
                        }}
                      ></div>
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-50 p-3 rounded-lg">
                    <div className="flex justify-between mb-1">
                      ...
                      <span className="text-sm font-bold">
                        {matchScore.details.workExperience.score}%
                      </span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div
                        className={`h-2 rounded-full ${
                          matchScore.details.workExperience.score > 70
                            ? "bg-green-500"
                            : matchScore.details.workExperience.score > 40
                            ? "bg-yellow-500"
                            : "bg-red-500"
                        }`}
                        style={{
                          width: `${matchScore.details.workExperience.score}%`,
                        }}
                      ></div>
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-50 p-3 rounded-lg">
                    <div className="flex justify-between mb-1">
                      ...
                      <span className="text-sm font-bold">
                        {matchScore.details.skills.score}%
                      </span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div
                        className={`h-2 rounded-full ${
                          matchScore.details.skills.score > 70
                            ? "bg-green-500"
                            : matchScore.details.skills.score > 40
                            ? "bg-yellow-500"
                            : "bg-red-500"
                        }`}
            ...
            {keywords.length > 0 && (
              <div className="mb-6">
                <h3 className="text-lg font-semibold mb-2">
                  Cuvinte cheie identificate
                </h3>
                <div className="flex flex-wrap gap-2">
                  {keywords.map((keyword, index) => {
                    const isInResume = JSON.stringify(resume)
                      .toLowerCase()
                      .includes(keyword);
                    return (
                      <span
          ...
          <div className="bg-white p-6 rounded-xl shadow-lg">
            <h2 className="text-xl font-semibold mb-4">
              Previzualizare CV optimizat pentru ATS
            </h2>
            <div
